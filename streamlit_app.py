...
class DynamiCrafterUI:
    ...
    def generate_video(self, filename, i2v_input_text, i2v_steps=50, i2v_cfg_scale=7.5, i2v_eta=1.0, i2v_motion=15, i2v_seed=123):
        data = {
            "image_name": filename,
    ...
    def run(self):
        ...
        i2v_end_btn = st.button("Generate")
        if i2v_end_btn:
            filename = self.upload_image(i2v_input_image.read())["filename"]
            if filename:
                video = self.generate_video(filename, i2v_input_text, i2v_steps, i2v_cfg_scale, i2v_eta, i2v_motion, i2v_seed)
                if video:
                    st.video(video)
                else:
